...
void Game::pollEvent()
{
    while (window->pollEvent(event))
    {
        ...
        case(sf::Event::MouseButtonPressed):
        {
            ...
            if (event.mouseButton.button == sf::Mouse::Left && isSelected == true && isMoving == true)
            {
                ...
                //std::cout << destinationPosition.x << ", " << destinationPosition.y << std::endl;

                //reseting the value of 'isSelect' and 'isMoving'

                MovePiece(selectionPosition, destinationPosition);
                ...
                for (int i = 0; i < 32; i++)
                {
                    for (int j = 0; j < 32; j++)
                    {
                        ...
                        if ( pieces_currentPosition[i] == pieces_currentPosition[j] && piecesList[i]->getIsAttacker() == true)
                        {
                            piecesList.erase(piecesList.begin()+(j-1));
                            pieces_currentPosition.erase(pieces_currentPosition.begin() + (j - 1));
                            priceList.resize(31);
                            piecesList[i]->isAttacker(false);
                        }
                    }
                }
